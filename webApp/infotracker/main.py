...
from InfoTrackerUser import User as itUser
...
class RegisterHandler(webapp2.RequestHandler):
	...
	def post(self):
		...
		username = self.request.get('username')
		password = self.request.get('password')
		check_user = itUser.query(itUser.username == username).get()
		if check_user:
			self.response.write(json.dumps({'statusCode': statusCode}))
			return
		newUser = itUser(name = name, username = username, password = password)
		if newUser:
			newUser.put()
			statusCode = 200
...
class WeightHandler(webapp2.RequestHandler):
	def get(self):
		...
		if not date:
			isDateSent = False
		all_weight = []
		logging.info(isDateSent)
		if not isDateSent:
			for weight in itWeight.query(itWeight.username == username):
				all_weight.append(weight)
			all_weight = sorted(all_weight,key=lambda r: r.date)
		logging.info(all_weight)
		return_list = []
		for weight in all_weight:
			return_list.append({'weight': weight.weight, 'date': weight.date.isoformat()})
